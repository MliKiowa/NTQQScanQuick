...
print('[debug] Maybe AES Encrypt Text: ',hex(offset_text_aes_encrypt))
...
for offset_aes_encrypt in list_offset_aes_encrypt_xref:
    ...
    if rva:
        print('[debug] AES Encrypt Function: ',hex(rva['start']))
        if rva['start'] in list_offset_aes_encrypt_func:
            continue
        list_offset_aes_encrypt_func.append(rva['start'])
print('[result] AES Encrypt Function: ',hex(list_offset_aes_encrypt_func[len(list_offset_aes_encrypt_func)-1]))

# 分析AES Decrypt Function
offset_text_aes_decrypt = search_bytes(pe, rdata_start, rdata_end, b'AES_gcm_256_decrypt')
print('[debug] Maybe AES Decrypt Text: ',hex(offset_text_aes_decrypt))
...
for offset_aes_decrypt in list_offset_aes_decrypt_xref:
    ...
    if rva:
        print('[debug] AES Decrypt Function: ',hex(rva['start']))
        if rva['start'] in list_offset_aes_decrypt_func:
            continue
        list_offset_aes_decrypt_func.append(rva['start'])
print('[result] AES Decrypt Function: ',hex(list_offset_aes_decrypt_func[len(list_offset_aes_decrypt_func)-1]))